import type { MeetupRequestBody } from '~/utils/meetup';
import { parseMeetupGroupResponse } from '~/utils/meetup';
...
async function getMeetupGroupInfo(url: string) {
    ...
    return fetch(MEETUP_API, {
        ...
        .then((res) => {
            ...
            throw new Error(
                `Error fetching from Meetup API: ${res.statusText}`
            );
        })
...
export async function tryToFetchRemixAustinInfo() {
    return getMeetupGroupInfo(REMIX_AUSTIN_MEETUP_URL)
        ...
        .catch((e) => {
            console.warn(
                `Error retrieving Meetup info (%s). Ignoring.`,
                e.message
            );
            return undefined;
        });
